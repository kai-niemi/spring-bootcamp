...
import io.cockroachdb.bootcamp.patterns.parallel.ProductVariationRepository;
import io.cockroachdb.bootcamp.patterns.parallel.SumInventoryTask;
import io.cockroachdb.bootcamp.test.AbstractIntegrationTest;

@SpringBootTest(classes = {Chapter4Application.class})
public class ParallelQueryTest extends AbstractIntegrationTest {
    private static final int PRODUCT_INVENTORY = 250;

    private static final int NUM_PRODUCTS_PER_COUNTRY = 500_000;
    ...
    public void beforeAll() {
        executionTime(() -> {
            ...
            COUNTRIES.forEach(country -> {
                ...
                productVariationRepository.insertProducts(PRODUCT_INVENTORY,
    ...
    public void whenExecutingAggregationQuery_thenExpectTotalSum() {
        BigDecimal total = executionTime(() -> {
            ...
            return productVariationRepository.sumInventory();
        }, (sum, duration) ->
        ...
        BigDecimal total = executionTime(() -> {
            try (ForkJoinPool forkJoinPool = ForkJoinPool.commonPool()) {
                ...
                SumInventoryTask sumInventoryTask
                        = new SumInventoryTask(COUNTRIES, productVariationRepository);
