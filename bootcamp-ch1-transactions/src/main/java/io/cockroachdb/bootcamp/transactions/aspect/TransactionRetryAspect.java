 ...
 * <p>
 * This aspect is separate from {@link TransactionAttributeAspect} which allows it to
 ...
 * and before the {@link TransactionAttributeAspect} if used simultaneously.
...
public class TransactionRetryAspect {
    public static <A extends Annotation> A findAnnotation(Signature signature, Class<A> annotationType) {
    ...
    @Around(value = "io.cockroachdb.bootcamp.aspect.Pointcuts.anyRetryableOperation(retryable)",
    ...
    public Object doAroundRetryableOperation(ProceedingJoinPoint pjp, Retryable retryable) throws Throwable {
        ...
        if (retryable == null) {
            retryable = findAnnotation(pjp.getSignature(), Retryable.class);
        }
        ...
        do {
            ...
            try {
                ...
                if (methodCalls > 1) {
                    retryHandler.afterRetry(pjp.getSignature(), throwable, methodCalls,
            ...
            if (retryHandler.shouldRetry(pjp.getSignature(), cause)) {
                retryHandler.beforeRetry(pjp.getSignature(), cause, methodCalls, retryable.delay());
            } else {
                throw throwable;
            }
        } while (methodCalls - 1 < retryable.maxRetries());
